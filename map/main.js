...
var mapboxSketch03 = function() {
  ...
  const map3 = new mapboxgl.Map({
      ...
      style: 'mapbox://styles/sfduncan/cmdcgfzp300ag01s2b90o5txd',
  ...
  map3.on('load', () => {
      ...
      fetch('SurveillanceCameras.geojson')
          .then(response => {
              if (!response.ok) {
          ...
          .then(data => {
              ...
              map3.addSource('SurveillanceCameras.geojson', {
                  'type': 'geojson',
                  'data': data
              });
              ...
              // Add a layer to visualize the points (customize as needed)
            map3.addLayer({
                'id': 'surveillance-layer',
                'type': 'circle',  // or 'symbol', 'fill', etc. depending on geometry
